  ...
  return (
    <div className="xl:w-[600px] hidden lg:inline ml-8 space-y-5 width-[10%]">
      <div className="width-[90%] xl:w-[75%] sticky top-0 bg-white py-1.5 z-50 space-y-5 ">
        ...
        <div className="text-gray-700 space-y-5 bg-gray-100 rounded-xl pt-2 width-[90%] xl:w-[99%]">
          <h4 className="font-bold text-xl px-4">What happening?</h4>
          <AnimatePresence>
          {newsResults.slice(0, articleNum).map((article) => (
            <motion.div key={article.title} initial={{opacity:0}} animate={{opacity:1}}
            exit={{opacity:0}}
        ...
        <div className="sticky top-16 text-gray-700 space-y-3 bg-gray-100 pt-2 rounded-xl w-[90%] xl:w-[99%]">
          <h4 className="font-bold text-xl px-4">Who to follow</h4>
          <AnimatePresence>
        
           
          {randomUsersResults.slice(0, randomUserNum).map((randomUser) => (
            <motion.div key={randomUser.login.username} initial={{opacity:0}} animate={{opacity:1}}
            exit={{opacity:0}}
            transition={{duration:1}}>
